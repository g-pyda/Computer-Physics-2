...
// ========== POTENTIAL ASSEMBLY AND TIME EVOLUTION ========= //
...
void time_evolution(int total_nodes, std::vector<double>& c_initial, 
                    ...
                    std::vector<std::vector<double>>& V,
                    double dt, double omega, double eF) {
    
    int n = total_nodes;
    std::vector<complexd> c(n);
    for(int i=0; i<n; ++i) c[i] = c_initial[i]; // Stan podstawowy t=0 

    complexd i_unit(0.0, 1.0);
    double h_bar = 1.0; // Jednostki atomowe

    for (double t = 0; t < T_max; t += dt) {
        // Macierz H(t) = H0 + eF * V * sin(omega * t) [cite: 32]
        // Zbuduj macierze A i B dla układu: [S - (dt/2hi)H(t+dt)]c(t+dt) = [S + (dt/2hi)H(t)]c(t) 
        
        // Rozwiąż układ równań liniowych (np. ZGESV z LAPACK)
        
        // Monitoruj normę: N(t) = c* O c [cite: 35]
        // Monitoruj rzuty: p_i(t) = c_eigen_i^T O c(t) [cite: 35]
    }
}
